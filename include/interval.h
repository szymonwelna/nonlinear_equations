...
template<typename T> Interval<T> ILog(const Interval<T> &x);
template<typename T> Interval<T> INeg(const Interval<T> &x);
...
public:
	...
	friend Interval ILog<T>(const Interval<T> &x);
	friend Interval INeg<T>(const Interval<T> &x);
...
template<typename T>
Interval<T> ILog(const Interval<T> &x) {
    int st = 0;
    Interval<T> one(1.0, 1.0);
    Interval<T> zero(0.0, 0.0);
    Interval<T> result = zero;
    Interval<T> term = one;
    int k = 1;
    T eps = 1E-18;
    ...
    if (x.a <= 0 || x.b <= 0) { 
        throw runtime_error("Logarithm of an interval containing zero or negative values.");
    }
    do {
        term = IMul(term, IDiv(ISub(x, one), Interval<T>(k, k)));
        result = IAdd(result, term);
        k++;
        if (IntWidth(result) < eps) {
            break;
        }
    } while (k < 10000);
...
template<typename T>
Interval<T> INeg(const Interval<T> &x) {
    return Interval<T>(-x.b, -x.a); 
}
...
template<typename T>
inline bool Interval<T>::operator ==(const Interval<T> &y) const {
